...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        batch_parses = unfinished_parses[:batch_size].copy()  # batch_size个句子
        transition = model.predict(batch_parses)
        for i, parse in enumerate(batch_parses):  # 对每个句子进行解析
            parse.parse_step(transition[i])
            if len(parse.stack) == 1 and len(parse.buffer) == 0:
                unfinished_parses.remove(parse)  # 移除unfinished_parses列表中parse的第一个匹配项，直到所有parse解析完毕，循环结束
